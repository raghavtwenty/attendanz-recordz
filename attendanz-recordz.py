"""
Designed and Programmed by Raghav, 
Coimbatore Institute Of Technology,
M. Sc. Artificial Intelligence and Machine Learning [Batch - 2021]

Date Created : 20 December 2020 | Last Updated : Sept 5, 2023
"""
...
if User_Home_Dir.exists() == True:
	if os.path.isfile(User_Home_File) == False : default_file_location()
else:
	try:
		# ----- If Not Exist Create New Dir And File -----
		User_Home_Dir.mkdir()
	finally : default_file_location()
...
def about_button():
    ...
    if about_button:
        ...
        tk.Label(about_win, text = 'Take online meetings attendance FASTER THAN EVER!').pack()

        # ----- About Window Basic Information ----- 
        tk.Label(about_win, text = 'App Name: attendanz Recordz').place(x = 0, y = 70)
        tk.Label(about_win, text = 'Last Updated: Sept 5, 2023').\
            place(x = 0, y = 100)
        tk.Label(about_win, text = 'Version: 1.0').\
            place(x = 0, y = 130)
        tk.Label(about_win, text = 'Compatible with: Mac Os 10.1+ & Windows 7 and above').\
            place(x = 0, y = 160)
        tk.Label(about_win, text = '© Designed & Programmed by Raghav').\
            place(x = 0, y = 190)
        tk.Label(about_win, text = 'Coimbatore Institute Of Technology,').\
            place(x = 0, y = 210)
        tk.Label(about_win, text = 'M.Sc Artificial Intelligence & Machine Learning [Batch - 2021]').\
            place(x = 0, y = 230)
        tk.Label(about_win, text = 'Contact Developer:').\
            place(x = 0, y = 270)
        tk.Label(about_win, text = 'GitHub : @raghavtwenty').\
            place(x = 0, y = 300)
...
def selecting_subject():
    ...
    subjectchoosen['values'] = ('Computer Science', 'Office Meetings','Accountancy',
                                'Biology', 'Business','Chemistry','Economics', 'English',
                                'Mathematics', 'Physics','Science', 'Social Science', 
...
''')
            ...
            default_file_location(Excel_File_Location = xlsx_file_name) # Update home file location
...
def process():
        ...
        all_present_set_low_len = len(all_present_set_low)-1
...
def save_pdf():
    if save_pdf:
        ...
        # iterate absentees names
        for absentees_names_iterate in absentees_names:
            pdf.cell(200, 10, txt = absentees_names_iterate.upper(), ln = 15, align = 'L')
...
'Designed & Programmed by Raghav', 
'Coimbatore Institute of Technology M.Sc. AI & ML [Batch - 2021]',
'Contact Developer : GitHub @raghavtwenty'
]
...
tk.Label(root, text = 'Hello 👋, Welcome To attendanz Recordz !', font = ('Arial', 20)).pack()
...
students_entry = tk.Label(root,text = 'Enter/Paste Current Participants Names* : ').\
    place(x = 0, y = 210)
...
# ----- Me! -----
tk.Label(root, text = '© Designed & Programmed by Raghav, GitHub : @raghavtwenty').\
    place(x = 350, y = 690)
